...
check: setup
	mkdir -p "$(MPLCONFIGDIR)"
	MPLCONFIGDIR="$(MPLCONFIGDIR)" $(PYTHON) -c "from blink import APILivenessDetector; from kyc import OCRGatewayClient; detector = APILivenessDetector(); gateway = OCRGatewayClient(); print(f'import ok ({detector.backend}); OCR gateway: {gateway.base_url}')"

test: setup
	MPLCONFIGDIR="$(MPLCONFIGDIR)" $(PYTHON) -m unittest discover -s tests
